classdef MorphCube < handle
    ...
    properties (Constant)
        ...
        voxel_dim = [3 3 3]   % int 3 x 1: no. of cubes in each dimension (default 5 x 5 x 5)
        ...
        function obj = MorphCube(chromosomes, mat_sorted_indcs, p_init_offset, v_init, a_init)
            ...
            if nargin ~= 0
                ...
                for n = size(chromosomes, 3):-1:1
                    ...
                    obj(n).voxel_pos = [X(:), Y(:), Z(:)];
                    % move the origin to the voxel center
                    obj(n).voxel_pos = obj(n).voxel_pos - min(obj(n).voxel_pos) - range(obj(n).voxel_pos)/2;
                    ...
                    % n_matrl
                    pd = zeros(size(obj(n).chromosome, 1), size(obj(n).voxel_pos, 1));
                    
                    % convert normalized values of mu to the xyz positions
                    MU = obj(n).chromosome(:, 1:3).*range(obj(n).voxel_pos) + min(obj(n).voxel_pos);
                    ...
                    for i = 1:size(obj(n).chromosome, 1)
                        ...
                        pd(i, :) = mvnpdf(obj(n).voxel_pos, MU(i, :), covar);
                    ...
                    % properties
                    obj(n).voxel_matrl = reshape(matrl, obj(n).voxel_dim(1), obj(n).voxel_dim(2), obj(n).voxel_dim(3));
                    ...
                    cube_corner = obj(n).cube_length*ones(1,3)/2;
                    ...
                    non_empty_indcs = find(obj(n).voxel_matrl(:) < 5); % array of non-empty material
                    ...
                    for i = 1:length(non_empty_indcs)
                        vox_center = obj(n).voxel_pos(non_empty_indcs(i),:);
                        ...
                        for j = 1: size(p_candidate)
                            ...
                            for k = 1: size(neighber_rel_pos, 1)
                                ...
                                if isempty(spring_connect_indcs) || ~ismember(candidate_connect_indcs, spring_connect_indcs, 'rows')
                                    ...
                                    acts = [acts; b_breathing*[new_L0/2, obj(n).omega, phase]]; %#ok<AGROW>
                    ...
                    obj(n).masses = PointMass(repmat(obj(n).mass, size(p, 1), 1), p, repmat(v_init(n ,:), size(p,1), 1), repmat(a_init(n, :), size(p,1), 1));
                    obj(n).springs = Spring(L_0, K, spring_connect_indcs, acts);
